...
Config lives in /etc/certheim/integrations.conf (INI), managed from the
...
CONFIG_PATH = Path("/var/opt/certheim/integrations.conf")
API_TIMEOUT = 15
...
def _api(method, path, fields=None, token=None, full_url=None):
    ...
    headers = {"PRIVATE-TOKEN": tok, "User-Agent": "certheim/2.3"}
    if data:
...
def save_settings(d):
    ...
    try:
        with open(CONFIG_PATH, "w") as f:
            f.write("# /etc/certheim/integrations.conf - managed via the admin UI\n")
            cfg.write(f)
    except OSError as e:
