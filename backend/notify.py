...
Reads config from /etc/certheim/email.conf at import time. All
...
CONFIG_PATH = Path("/var/opt/certheim/email.conf")
...
def save_settings(d):
    ...
    api_key/password keeps the stored one). File must be certheim-writable."""
    cfg = configparser.ConfigParser()
    if CONFIG_PATH.exists():
        try:
            cfg.read(CONFIG_PATH)
    ...
    try:
        with open(CONFIG_PATH, "w") as f:
            f.write("# /etc/certheim/email.conf\n")
