#!/bin/bash
# certheim-uninstall.sh - cleanly remove the Certheim from this host.
#
#   sudo ./certheim-uninstall.sh            Guided uninstall (prompts before each
#                                      destructive choice; safe defaults).
#   sudo ./certheim-uninstall.sh --help     Show this help.
#   sudo ./certheim-uninstall.sh --dry-run  Show what WOULD be removed; change
#                                      nothing.
#
# What it does:
#   - stops + disables the systemd units (certheim-api, certheim-expiry-warn .service
#     and .timer) and removes the unit files
#   - removes the application code (/opt/certheim), frontend
#     (/var/www/csr), helper scripts, and the installed tools
#   - removes ONLY the app's nginx fragment (/etc/nginx/certheim.d/30-csr.conf);
...
#   - PROMPTS about removing the certheim service account
#
# What it deliberately does NOT touch:
#   - PKI material (/etc/pki/certheim, client CA bundle) - site-managed
...
WWW_DIR="/var/www/csr"
DB_DIR="/var/lib/certheim"
DB_FILE="${DB_DIR}/jobs.db"
CFG_DIR="/etc/certheim"
HELPER_DIR="/opt/certheim/helper"
NGINX_FRAG="/etc/nginx/certheim.d/30-csr.conf"
SUDOERS="/etc/sudoers.d/certheim"
SVC_USER="certheim"
UNITS=(certheim-api.service certheim-expiry-warn.service certheim-expiry-warn.timer)
TOOLS=(/usr/local/sbin/certheim-backup /usr/local/sbin/certheim-bootstrap-admin)
...
say "5) config"
if [[ -d "$CFG_DIR" ]]; then
    # email.conf / certheim.env are operator state; back them up rather
    # than silently deleting credentials/relay settings.
    if $DRY; then
        echo "  [dry-run] would back up + remove $CFG_DIR"
    else
        ts="$(date +%Y%m%d-%H%M%S)"
        bk="/root/certheim-config-backup-${ts}"
