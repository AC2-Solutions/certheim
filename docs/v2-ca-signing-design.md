...
4. **Role:** `pki_csr/roles/certheim` — `allowed_domains`,
...
5. **Scoped app credential:** an ACL policy that allows **only**
   `pki_csr/sign/certheim` (`update`) — no `issue`, no `root`/
...
- **New egress** — `gunicorn (certheim) → OpenBao` over TLS is new outbound on the
