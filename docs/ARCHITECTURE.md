...
## Ops & packaging
- `deploy.sh` — installs repo files to live paths from a `MANIFEST` (idempotent, validates units/nginx, restarts `csr-api`, checks served VERSION). **Edit the MANIFEST by hand** (a quoting slip via `sed` once corrupted the array).
...
4. deploy to **csr-dev** → `verify.sh` clean (`missing=0 drift=0`) → csr-api active.
