...
def test_db_shim_logic():
    """The pluggable-backend shim's pure logic (no live DB): placeholder + '%'
    escaping, DDL dialect translation, and the hybrid Row. SQLite end-to-end is
    covered by the whole suite; Postgres end-to-end is the smoke-tests-postgres
    CI job (needs a runner with service containers)."""
    import db as dbx
    try:
        dbx.configure(backend="postgres", dsn="postgresql://x")
        assert dbx.is_postgres()
        # '?' -> '%s' and literal '%' doubled (psycopg printf-style binding)
        assert dbx._translate("WHERE a = ? AND b LIKE '%x%'") == "WHERE a = %s AND b LIKE '%%x%%'"
        # AUTOINCREMENT integer pkey -> identity; epoch REAL -> double precision
        d = dbx.ddl("CREATE TABLE z (id INTEGER PRIMARY KEY AUTOINCREMENT, ts REAL, n TEXT)")
        assert "GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY" in d and "AUTOINCREMENT" not in d
        assert "DOUBLE PRECISION" in d
        assert dbx.ddl("CREATE INDEX i ON z(n)") == "CREATE INDEX i ON z(n)"  # untouched
        # hybrid Row: positional, by-name, dict(), membership, len, get-default
        r = dbx.Row(["id", "name"], (7, "acme"))
        assert r[0] == 7 and r["name"] == "acme" and dict(r) == {"id": 7, "name": "acme"}
        assert "name" in r and len(r) == 2 and r.get("missing", "d") == "d"
        # sqlite leaves DDL identical (zero behaviour change on the default path)
        dbx.configure(backend="sqlite")
