...
# server{} block. deploy.sh installs it to $NGINX_INCLUDE_DIR (csr-dashboard.d). For a
# fresh/standalone box (no pre-existing server doing that include, e.g. csr-host's
...
# fresh/air-gapped box. (On csr-host an equivalent block already exists in
