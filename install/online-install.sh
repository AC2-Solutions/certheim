...
#
#     sudo SERVICE_USER=certheim FQDN=cert.example.com \
...
cac_licensed() {
    ...
    [[ -n "$py" ]] || return 1
    CERTHEIM_LICENSE_FILE="$lf" "$py" -c "import sys; sys.path.insert(0,'backend'); import capabilities; sys.exit(0 if capabilities.is_entitled('auth.cac') else 1)" 2>/dev/null
}
...
# A. Identity & paths
ask SERVICE_USER  "Service account (the app runs as this user)" "certheim"
...
case "${DB_BACKEND,,}" in
  postgres|postgresql)
    DB_BACKEND=postgres
    ask CERTHEIM_DB_HOST     "  Postgres host" "localhost"
    ask CERTHEIM_DB_PORT     "  Postgres port" "5432"
    ask CERTHEIM_DB_NAME     "  Postgres database" "certheim"
    ask CERTHEIM_DB_USER     "  Postgres user" "certheim"
    ask CERTHEIM_DB_PASSWORD "  Postgres password" ""
    ask CERTHEIM_DB_SSLMODE  "  Postgres sslmode (disable/require/verify-full)" "require"
    ;;
  *) DB_BACKEND=sqlite ;;
esac
...
if is_yes "$CONFIGURE_EMAIL"; then
    ...
    ask FROM_ADDRESS "  From address" "noreply-certheim@example.com"
...
if is_yes "$CONFIGURE_OPENBAO"; then
    ask CERTHEIM_OPENBAO_ADDR    "  OpenBao address" "https://openbao.example.com"
    ask CERTHEIM_OPENBAO_ROLE_ID "  AppRole role_id" ""
    ask_secret CERTHEIM_OPENBAO_SECRET_ID "  AppRole secret_id"
fi

# Derived / fixed paths (internal identifiers kept per the rebrand).
CERT_DIR=/etc/pki/certheim
NGINX_INCLUDE_DIR=/etc/nginx/certheim.d
SMG_HOST="${SMG_HOST:-smtp.example.com}"; SMG_PORT="${SMG_PORT:-25}"
FROM_ADDRESS="${FROM_ADDRESS:-noreply-certheim@example.com}"; GLOBAL_CC="${GLOBAL_CC:-}"
...
echo "  openbao         : $(is_yes "$CONFIGURE_OPENBAO" && echo "${CERTHEIM_OPENBAO_ADDR}" || echo "(skip)")"
...
# ---------------------------------------------------------------------------
DIRS=(
  "/opt/certheim|root:${SERVICE_GROUP}|0750"
  "/var/lib/certheim|${SERVICE_USER}:${SERVICE_GROUP}|0750"
  "/var/www/csr|root:nginx|0750"
  "/etc/certheim|root:${SERVICE_GROUP}|0750"
  "/opt/certheim/helper|root:root|0750"
  "/opt/certheim/helper/certheim_helper.d|root:root|0750"
  "/var/opt/certheim|root:root|0755"
  "/var/opt/certheim/private|root:root|0700"
  "/var/opt/certheim/requests|root:${SERVICE_GROUP}|0750"
  "/var/opt/certheim/issued|${SERVICE_USER}:${SERVICE_GROUP}|0750"
)
...
# no PyPI reach. CERTHEIM_WHEELHOUSE overrides the location. When absent we fall
...
WHEELHOUSE="${CERTHEIM_WHEELHOUSE:-$(pwd)/wheelhouse}"
PIP_OFFLINE=()
...
chmod 0750 /opt/certheim "$VENV" "$VENV/bin"
chmod -R g+rX "$VENV"
...
EMAILCONF
chown "${SERVICE_USER}:${SERVICE_GROUP}" /etc/certheim/email.conf
chmod 0640 /etc/certheim/email.conf

ENVF=/etc/certheim/certheim.env
if [[ ! -f "$ENVF" && -f config/certheim.env.example ]]; then
    install -o "$SERVICE_USER" -g "$SERVICE_GROUP" -m 0640 \
        config/certheim.env.example "$ENVF"
fi
...
set_env CERTHEIM_AUTH_MODE "$AUTH_MODE"
# Database backend. For postgres, CERTHEIM_DB_BACKEND=postgres makes the app select
# PG regardless of the (ignored) CERTHEIM_DB_PATH; the discrete CERTHEIM_DB_* parts build
# the libpq DSN. SQLite is the default and needs no extra env (CERTHEIM_DB_PATH is
# already in the example).
if [[ "$DB_BACKEND" == "postgres" ]]; then
    set_env CERTHEIM_DB_BACKEND  postgres
    set_env CERTHEIM_DB_HOST     "$CERTHEIM_DB_HOST"
    set_env CERTHEIM_DB_PORT     "$CERTHEIM_DB_PORT"
    set_env CERTHEIM_DB_NAME     "$CERTHEIM_DB_NAME"
    set_env CERTHEIM_DB_USER     "$CERTHEIM_DB_USER"
    set_env CERTHEIM_DB_PASSWORD "$CERTHEIM_DB_PASSWORD"
    set_env CERTHEIM_DB_SSLMODE  "$CERTHEIM_DB_SSLMODE"
fi
...
set_env CERTHEIM_BOOTSTRAP_FIRST_ADMIN 1
if is_yes "${CONFIGURE_OPENBAO:-no}"; then
    set_env CERTHEIM_OPENBAO_ADDR "${CERTHEIM_OPENBAO_ADDR:-}"
    set_env CERTHEIM_OPENBAO_ROLE_ID "${CERTHEIM_OPENBAO_ROLE_ID:-}"
    set_env CERTHEIM_OPENBAO_SECRET_ID "${CERTHEIM_OPENBAO_SECRET_ID:-}"
    set_env CERTHEIM_CAP_OPENBAO 1
fi
[[ -n "${LICENSE_FILE:-}" && -r "$LICENSE_FILE" ]] && set_env CERTHEIM_LICENSE_FILE "$LICENSE_FILE"
# Local mode needs a persistent session signing key.
if [[ "$AUTH_MODE" == "local" && ! -s /etc/certheim/secret.key ]]; then
    openssl rand -hex 32 > /etc/certheim/secret.key
    chown "${SERVICE_USER}:${SERVICE_GROUP}" /etc/certheim/secret.key
    chmod 0640 /etc/certheim/secret.key
fi
...
setup_tls() {
    case "${TLS_MODE,,}" in
      ...
      stepca)
        ...
        cat > /etc/default/certheim-tls-renew <<RENEW
CERT_LOCATION=$CERT_DIR/server.crt
KEY_LOCATION=$CERT_DIR/server.key
RENEW
        install -m 0644 systemd/certheim-tls-renew.service /etc/systemd/system/certheim-tls-renew.service
        install -m 0644 systemd/certheim-tls-renew.timer   /etc/systemd/system/certheim-tls-renew.timer
        systemctl daemon-reload
        systemctl enable --now certheim-tls-renew.timer
...
SERVER_CONF=/etc/nginx/conf.d/certheim.conf
...
# ---------------------------------------------------------------------------
bash ./deploy.sh
systemctl enable certheim-api.service certheim-expiry-warn.timer certheim-auto-renew.timer >/dev/null 2>&1 || true
systemctl start certheim-expiry-warn.timer certheim-auto-renew.timer 2>/dev/null || true
...
then
    [[ "$DB_BACKEND" == "sqlite" ]] && \
        chown "${SERVICE_USER}:${SERVICE_GROUP}" /var/lib/certheim/jobs.db 2>/dev/null || true
    # deploy.sh started the service BEFORE this seed - restart so it reads the
    # seeded auth_mode/registration settings.
    systemctl try-restart certheim-api.service 2>/dev/null || true
else
...
echo ""
log "Post-install health check (certheim-doctor)"
bash "$REPO_ROOT/tools/certheim-doctor.sh" || warn "certheim-doctor reported problems above - review before using this host"
